...
#include "maps_manager_pkg/world_to_octomap_plugin.h"
...
bool OctomapFromGazeboWorld::ServiceCallback(
    maps_manager_pkg::Octomap::Request& req, maps_manager_pkg::Octomap::Response& res) {
  ...
  CreateOctomap(req);
  if (req.filename.data != "") {
    if (octomap_) {
      std::string path = req.filename.data;
  ...
  res.origin_latitude.data = origin_spherical.X();
  res.origin_longitude.data = origin_spherical.Y();
  res.origin_altitude.data = origin_spherical.Z();
  return true;
}
...
void OctomapFromGazeboWorld::FloodFill(
  ...
  while (to_check.size() > 0) {
    ...
    if ((p.X() > bounding_box_origin.X() - bounding_box_lengths.X() / 2) &&
      ...
      octomap_->updateNode(p.X(), p.Y(), p.Z(), false); // Example to set as free
      to_check.pop();
...
void OctomapFromGazeboWorld::CreateOctomap(
    const maps_manager_pkg::Octomap::Request& msg) {
  ...
  double leaf_size = msg.leaf_size.data;
